...
MD5Sum::MD5Sum(const string& sSource)
{
	Calculate(sSource);
}

MD5Sum::MD5Sum(const unsigned char* pachSource, unsigned int nLen)
{
	Calculate(pachSource, nLen);
}

string MD5Sum::Calculate(const string& sSource)
{
	return Calculate((const unsigned char*)sSource.c_str(), sSource.size());
}

string MD5Sum::Calculate(const unsigned char* pachSource, unsigned int nLen)
{
	MD5_CTX context;
...
#if defined(__GNUC__)
		sprintf(temp, "%02x", m_rawHash[i]);
#else
		sprintf_s(temp, "%02x", m_rawHash[i]);
#endif
