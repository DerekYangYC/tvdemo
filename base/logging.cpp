...
: impl_(toAbort ? FATAL : ERRORS, errno, file, line)
{
}
...
char* FormLog(int &index,char *szbuf, size_t size_buf, unsigned char* buffer, size_t size)
{
	...
	while (size > lsize && len + 10 < size_buf)
	{
		if (lsize % 32 == 0)
		{
...
#else
			strncpy_s(szhead, ullto4Str(index++), sizeof(szhead) - 1);
#endif
			...
			strcat(szbuf, szhead);
#else
			strcat_s(szbuf, size_buf, szhead);
#endif
			len += headlen;
...
void Logger::Log(const char* sFormat, ...)
{
	va_list ap;
	va_start(ap, sFormat);
	char temp[1024 * 1024] = { 0 };
	vsnprintf(temp, sizeof(temp) - 1, sFormat, ap);
	va_end(ap);
